...
ssh -T ubuntu@192.168.1.10 << EOF
  ...
  sudo docker run -v /opt/cenozoa/nginx.conf:/etc/nginx/conf.d/default.conf -v /opt/cenozoa/.htpasswd:/etc/apache2/.htpasswd -v /opt/cenozoa/.supasswd:/etc/apache2/.supasswd -v /etc/letsencrypt:/etc/ssl/letsencrypt -v /opt/cenozoa/ui:/www -p 443:443 -p 80:80 --network=cenozoanet --name=nginx --rm -d nginx
EOF
